...
namespace sadistic {
    ...
    struct SadisticUnlockForm::OverlayComp  : public juce::Component, private juce::Thread, private Timer {
...
//            g.setFont (sadistic::getSadisticFont());
    ...
    String SadisticUnlockForm::getMessage() {
    ...
    SadisticUnlockForm::SadisticUnlockForm (SadisticMarketplaceStatus& s) : status (s), message (String(), getMessage()) {
        ...
        if(isPlugin()) {
            ...
            for (int i = 1; i < (int)sizeof(status.productName); i++) productList.addItem(status.productName[i].data(), i);
        }
    ...
    SadisticUnlockForm::~SadisticUnlockForm() { unlockingOverlay.deleteAndZero(); }
...
//        logo->centreWithSize((int)((float) getHeight() * 0.93f), (int)((float) getHeight() / 0.93f));
    ...
    void SadisticUnlockForm::showBubbleMessage (const String& text, Component& target) {
    ...
    void SadisticUnlockForm::attemptRegistration() {
