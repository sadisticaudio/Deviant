...
namespace sadistic {
    ...
    template <typename... SRefs> void listenToTheseParameters (APVTS& apvts, APVTS::Listener* listener, SRefs&... srefs) {
        forEach ([&] (auto& sref) { apvts.addParameterListener(sref, listener); }, srefs...); }
    template <typename... Procs>
...
//        static constexpr std::string_view waveID[] {
//            "none",
//            "sine",
//            "cosine",
//            "square",
...
//    };
    ...
    //I am afraid to use this hand-rolled delay in place of the JUCE version but i seem to get a huge performance penalty with the JUCE version, even in Release mode, might be my config...
    template<typename FloatType>
    struct DelayBuffer {
...
//            jassert (destBuffer.getNumSamples() == buffer.getNumSamples());
            //destBuffer.makeCopyOf(buffer, true);
            for (int j { 0 }; j < channels; ++j) {
